const desc =
  "Real Madrid Club de Fútbol, more commonly known as Real Madrid, is a hugely popular Spanish soccer club based in Madrid. Founded in March 1902 as the Madrid Football Club, it is one of the most talented, wealthy and influential clubs in Spanish soccer, having achieved numerous successes at home and in European Cup competitions Their current stadium is the Santiago Bernabeu in Madrid. \n The first logo, created in 1902, featured the three letters . \n The logo itself was blue, while the shirts on which it appeared were white. Although the design was visually appealing and elegant, it was far from easy to read.";

const AlabaBio =
  "David Olatukunbo Alaba (born 24 June 1992) is an Austrian professional footballer who plays as a centre-back or left-back for La Liga club Real Madrid and captains the Austria national team. [5][6]\
Alaba started out in Bayern's youth system before being promoted to the reserve team for the 2009–10 season.[7] In January 2011, Alaba joined TSG 1899 Hoffenheim on loan until the end of the 2010–11 season. He returned to Bayern at the start of the 2011–12 season, where he went on to become a regular member of the first-team squad.";
const Alaba = {
  name: "Alaba",
  role: "defender",
  biography: AlabaBio,
  picture:
    "https://firebasestorage.googleapis.com/v0/b/adapter-dbd21.appspot.com/o/Real%20Madrid%2FPlayers%2FAlaba.png?alt=media&token=31f3ea8c-56ef-40d9-9c99-43c97e5b729c",
  stats: {
    placeOfBirth: "Viena (Austria)",
    dateOfBirth: "24/06/1992",
    clubStart: "-/-/2021",
    weight: "78kg",
    height: "180cm",
    marketValue: "€55.00m",
  },
};

///////////

const BenzemaBio =
  "Karim Mostafa Benzema (born 19 December 1987) is a French professional footballer who plays as a striker for and captains La Liga club Real Madrid. A creative and prolific forward, Benzema is regarded as one of the best strikers of all time.[4][5][6] He is Real Madrid's all-time second-highest goalscorer and top assist provider.[7] Benzema has won 24 trophies with Real Madrid, including four La Liga, two Copa del Rey and five UEFA Champions League titles.";
const Benzema = {
  name: "Benzema",
  role: "forward",
  biography: BenzemaBio,
  picture:
    "https://firebasestorage.googleapis.com/v0/b/adapter-dbd21.appspot.com/o/Real%20Madrid%2FPlayers%2FBenzema.png?alt=media&token=614e2777-ade5-4971-8934-bcc82a5fb7bb",
  stats: {
    placeOfBirth: "Lyon (France)",
    dateOfBirth: "19/12/1987",
    clubStart: "-/-/2009",
    weight: "81.2kg",
    height: "185cm",
    marketValue: "€35.00m",
  },
};

//////

const CamavingaBio =
  "Camavinga was born in a refugee camp in Cabinda, Angola in 2002, to Congolese parents from Brazzaville.[4][5] He has five siblings. His family moved to France when he was two.[6] They moved to Fougères, where he grew up. He practiced judo for a time, before giving it up to focus solely on football. ";
const Camavinga = {
  name: "Camavinga",
  role: "midfield",
  biography: CamavingaBio,
  picture:
    "https://firebasestorage.googleapis.com/v0/b/adapter-dbd21.appspot.com/o/Real%20Madrid%2FPlayers%2FCamavinga.png?alt=media&token=40c4e723-a9ad-4f7e-bed9-9df019b54b42",
  stats: {
    placeOfBirth: "Cabinda (Angola)",
    dateOfBirth: "10/11/2002",
    clubStart: "-/-/2021",
    weight: "68kg",
    height: "182cm",
    marketValue: "€50.00m",
  },
};

//////

const CarvajalBio =
  "Having risen through the Real Madrid youth ranks, he spent a season with Bayer Leverkusen before returning to Real Madrid. He broke into the first team in 2013, and has subsequently gone on to win 20 major trophies at Real Madrid including five Champions League trophies, three La Liga titles and a Copa del Rey.";
const Carvajal = {
  name: "Carvajal",
  role: "defender",
  biography: CarvajalBio,
  picture:
    "https://firebasestorage.googleapis.com/v0/b/adapter-dbd21.appspot.com/o/Real%20Madrid%2FPlayers%2FCarvajal.png?alt=media&token=56508813-8c10-4cc1-8dd5-b97e402adba2",
  stats: {
    placeOfBirth: "Leganés (Madrid)",
    dateOfBirth: "11/01/1992",
    clubStart: "-/-/2013",
    weight: "73,5kg",
    height: "173cm",
    marketValue: "€18.00m",
  },
};

//////

const CourtoislBio =
  "Courtois progressed through the youth system of Genk, and at age 18, he played a key role in the team's Belgian Pro League victory. In July 2011, he joined Chelsea for a reported £8 million, and was immediately loaned to Atlético Madrid. During his three seasons there, they won the Europa League in 2012, the Copa del Rey in 2013, and the La Liga title in 2014. ";
const Courtois = {
  name: "Courtois",
  role: "goalkeeper",
  biography: CourtoislBio,
  picture:
    "https://firebasestorage.googleapis.com/v0/b/adapter-dbd21.appspot.com/o/Real%20Madrid%2FPlayers%2FCourtois.png?alt=media&token=5794b606-0146-4e6e-a915-1e009ef8b3d2",
  stats: {
    placeOfBirth: "Bree (Belgique)",
    dateOfBirth: "11/05/1992",
    clubStart: "-/-/2018",
    weight: "96kg",
    height: "200cm",
    marketValue: "€60.00m",
  },
};

//////

const MilitãolBio =
  "Éder Gabriel Militão (Brazilian Portuguese: [ˈɛdɛʁ ɡabɾiˈɛw miliˈtɐ̃w]; born 18 January 1998) is a Brazilian professional footballer who plays for the La Liga club Real Madrid and the Brazil national team. Mainly a central defender, he can also play as a right back or defensive midfielder.[3]";
const Militão = {
  name: "Militão",
  role: "defender",
  biography: MilitãolBio,
  picture:
    "https://firebasestorage.googleapis.com/v0/b/adapter-dbd21.appspot.com/o/Real%20Madrid%2FPlayers%2FE.%20Milit%C3%A3o.png?alt=media&token=cfda9cf4-be53-482e-a01a-495130a990b4",
  stats: {
    placeOfBirth: "Sertãozinho (Brésil)",
    dateOfBirth: "18/01/1998",
    clubStart: "-/-/2019",
    weight: "78kg",
    height: "186cm",
    marketValue: "€70.00m",
  },
};

//////

const ModrićlBio =
  "Born in Zadar,[8] Modrić's childhood coincided with the Croatian War of Independence which displaced his family. In 2002, he was signed by Dinamo Zagreb at age 16, after showing promise with his hometown club NK Zadar's youth team. He continued his development in Zagreb, before spells on loan to Zrinjski Mostar and Inter Zaprešić. He made his debut for Dinamo in 2005 and won three consecutive league titles and domestic cups, being named the Prva HNL Player of the Year in 2007. In 2008, he moved to Premier League club Tottenham Hotspur for a club-record transfer fee of £16.5 million, where he led Spurs to their first UEFA Champions League appearance in almost 50 years, reaching the quarter-finals of the 2010–11 tournament.";

const Modrić = {
  name: "Modrić",
  role: "midfield",
  biography: ModrićlBio,
  picture:
    "https://firebasestorage.googleapis.com/v0/b/adapter-dbd21.appspot.com/o/Real%20Madrid%2FPlayers%2FModri%C4%87.png?alt=media&token=feff2cd6-c89c-44b7-a1f7-35d426cca94b",
  stats: {
    placeOfBirth: "Zadar (Croatie)",
    dateOfBirth: "09/09/1985",
    clubStart: "-/-/2012",
    weight: "66kg",
    height: "172cm",
    marketValue: "€10.00m",
  },
};

//////

const NachoBio =
  "Born in Madrid, Nacho arrived in Real Madrid's youth system at the age of 11. He made his senior debut in 2008–09, playing two games with the reserves in Segunda División B and subsequently appearing in a further two full seasons in that level; it was during this time he formed a long-lasting friendship with future Russian national team player Denis Cheryshev.[4]";
const Nacho = {
  name: "Nacho",
  role: "defender",
  biography: NachoBio,
  picture:
    "https://firebasestorage.googleapis.com/v0/b/adapter-dbd21.appspot.com/o/Real%20Madrid%2FPlayers%2FNacho.png?alt=media&token=4d986c6e-8310-4651-8384-3f89c421a3ff",
  stats: {
    placeOfBirth: "Madrid",
    dateOfBirth: "18/01/1990",
    clubStart: "-/-/2008",
    weight: "76,3kg",
    height: "180cm",
    marketValue: "€6.00m",
  },
};

//////

const RüdigerBio =
  "Antonio Rüdiger (born 3 March 1993) is a German professional footballer who plays as a centre-back for La Liga club Real Madrid and the Germany national team.\
Rüdiger began his career at VfB Stuttgart, representing their reserves in 3. Liga and the first team in the Bundesliga. In 2015 he joined Roma, initially on loan and a year later for a €9 million fee. He was signed by Chelsea in 2017 for an estimated £27 million, where he won the FA Cup in his first season, followed by the UEFA Europa League in his second season, and the UEFA Champions League in 2021. In 2022, Rüdiger signed for European champions Real Madrid.";

const Rüdiger = {
  name: "Rüdiger",
  role: "defender",
  biography: RüdigerBio,
  picture:
    "https://firebasestorage.googleapis.com/v0/b/adapter-dbd21.appspot.com/o/Real%20Madrid%2FPlayers%2FR%C3%BCdiger.png?alt=media&token=b6ef43e8-eb50-4fc3-9c9f-e23a0bd68519",
  stats: {
    placeOfBirth: "Berlin (Allemagne)",
    dateOfBirth: "03/03/1993",
    clubStart: "-/-/2022",
    weight: "86kg",
    height: "190cm",
    marketValue: "€40.00m",
  },
};

//////

const ValverdeBio =
  "Valverde spent most of his youth career with Peñarol, where he quickly made an impression; he made his senior debut for the club in their first fixture of the 2015–16 season against Cerro.[4] He became part of the Uruguayan national youth teams and attracted the interest of European clubs like Arsenal, Barcelona, Chelsea and Real Madrid.[5][6][7]";
const Valverde = {
  name: "Valverde",
  role: "midfield",
  biography: ValverdeBio,
  picture:
    "https://firebasestorage.googleapis.com/v0/b/adapter-dbd21.appspot.com/o/Real%20Madrid%2FPlayers%2FValverde.png?alt=media&token=f18359d5-e39e-43b7-ad5c-50032a77814e",
  stats: {
    placeOfBirth: "Montevideo (Uruguay)",
    dateOfBirth: "22/07/1998",
    clubStart: "-/-/2018",
    weight: "78kg",
    height: "182cm",
    marketValue: "€100.00m",
  },
};

const ViniBio =
  "Vinícius Junior's football career began in the year 2006, when his father took him to the branch offices of Flamengo, in the neighborhood of Mutuá, in São Gonçalo where he lived. His club document described him as a left-back.";
const Vini = {
  name: "Vinicius Paixao de Oliveira Junior",
  role: "forward",
  biography: ViniBio,
  picture:
    "https://firebasestorage.googleapis.com/v0/b/adapter-dbd21.appspot.com/o/Real%20Madrid%2FPlayers%2FVini%20Jr..png?alt=media&token=14bb23fb-d237-4b77-8dc0-bf8767d25128",
  stats: {
    placeOfBirth: "São Gonçalo (Río de Janeiro, Brasil)",
    dateOfBirth: "12/07/2000",
    clubStart: "-/-/2018",
    weight: "73kg",
    height: "186cm",
    marketValue: "€120.00m",
  },
};

const RealChampionsLeague = {
  ChampionsLeague: {
    picture:
      "https://firebasestorage.googleapis.com/v0/b/adapter-dbd21.appspot.com/o/Trophys%2Fchampions-league-png_9224938.png?alt=media&token=418c1c02-48df-474f-8821-db93b1013c87",
    number: "8",
  },
  EuropienLeague: {
    picture:
      "https://firebasestorage.googleapis.com/v0/b/adapter-dbd21.appspot.com/o/Trophys%2FEuropienLeague.png?alt=media&token=858070ac-8752-46d1-be04-765123f9f054",
    number: "6",
  },
  FifaClubWorldCup: {
    picture:
      "https://firebasestorage.googleapis.com/v0/b/adapter-dbd21.appspot.com/o/Trophys%2FfifaClubWorldCup.png?alt=media&token=e3c343bd-6387-4c1b-849e-af4e85a38532",
    number: "5",
  },
  SpanishCup: {
    picture:
      "https://firebasestorage.googleapis.com/v0/b/adapter-dbd21.appspot.com/o/Trophys%2FspanishCup.png?alt=media&token=2c5ffca0-6e28-47e2-8116-e173aa935325",
    number: "35",
  },
  UEAFcup: {
    picture:
      "https://firebasestorage.googleapis.com/v0/b/adapter-dbd21.appspot.com/o/Trophys%2FUEFAcup.png?alt=media&token=fbb7c7d1-08a5-4dba-b818-df0d9847a267",
    number: "5",
  },
};

const RealTeam = {
  name: "Real Madrid",
  logo: "https://firebasestorage.googleapis.com/v0/b/adapter-dbd21.appspot.com/o/Real%20Madrid%2FLogo%2Freal-logo.png?alt=media&token=666f2f50-86b7-4d5a-b8a7-94029116d8e4",
  description: desc,
  players: [
    Alaba,
    Benzema,
    Camavinga,
    Carvajal,
    Courtois,
    Militão,
    Modrić,
    Nacho,
    Rüdiger,
    Valverde,
    Vini,
  ],
  Trophys: RealChampionsLeague,

  Tshirts: {
    Tshirt1:
      "https://firebasestorage.googleapis.com/v0/b/adapter-dbd21.appspot.com/o/Real%20Madrid%2FTshirts%2FTshirt1.webp?alt=media&token=793d76e5-6f09-41be-a60a-3f3516ef2407",
    Tshirt2:
      "https://firebasestorage.googleapis.com/v0/b/adapter-dbd21.appspot.com/o/Real%20Madrid%2FTshirts%2FTshirt2.webp?alt=media&token=883bf097-0edd-4eb6-b307-d97afeb23fd2",
    Tshirt3:
      "https://firebasestorage.googleapis.com/v0/b/adapter-dbd21.appspot.com/o/Real%20Madrid%2FTshirts%2FTshirt3.webp?alt=media&token=33627516-5114-446b-9f6b-df4ec0c863fa",
  },
};

export default RealTeam;
